          ...
          <div className="details-flex">
            ...
            <div className="details-info">
              ...
              <div style={{ marginTop: '30px', display: 'flex', gap: '15px' }}>
                <button className="btn-checkout" onClick={() => handleBookingStart(viewingDetails)}>ĐẶT VÉ NGAY</button>
  ...
  if (selectedMovie && !selectedTheater) {
    return (
      <div className="booking-section" style={{ textAlign: 'left' }}>
        <button className="btn-back" onClick={handleBack}>← Quay lại chọn phim</button>
        <h2 style={{ color: '#fbbf24', textAlign: 'center', marginBottom: '30px' }}>
          Chọn rạp để xem: {selectedMovie.title}
        </h2>
        <div className="theater-grid">
          {THEATERS.map((t, i) => (
            <div key={i} className="theater-card" style={{ cursor: 'pointer' }} onClick={() => handleSelectTheater(t)}>
              <h3 style={{ marginBottom: '5px' }}>{t.name}</h3>
              <p style={{ color: '#a0aec0', fontSize: '0.9rem' }}>{t.address}</p>
              <div style={{ marginTop: '15px', color: '#fbbf24', fontSize: '0.9rem' }}>● Nhấn để chọn rạp này</div>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="booking-section">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <button className="btn-back" style={{ marginBottom: 0 }} onClick={handleBack}>← Đổi rạp</button>
          <div style={{ textAlign: 'right' }}>
            <h3 style={{ margin: 0, color: '#fbbf24' }}>{selectedMovie.title}</h3>
            <p style={{ margin: 0, fontSize: '0.9rem', color: '#a0aec0' }}>Tại: {selectedTheater.name}</p>
          </div>
        </div>
        ...
        <div className="legend">
          <div className="legend-item"><div className="legend-dot" style={{ background: '#374151' }}></div>Trống</div>
          <div className="legend-item"><div className="legend-dot" style={{ background: '#fbbf24' }}></div>Đang chọn</div>
          <div className="legend-item"><div className="legend-dot" style={{ background: '#e50914' }}></div>Đã bán</div>
        </div>
        <div className="summary">
          ...
          <button className="btn-checkout" disabled={selectedSeats.length === 0}
            onClick={() => alert(`Thanh toán thành công!\nPhim: ${selectedMovie.title}\nRạp: ${selectedTheater.name}\nTổng tiền: ${totalPrice.toLocaleString()}đ`)}>
            THANH TOÁN
          </button>
        </div>
      </div>
  ...
  return (
    <>
      <div className="search-container">
        <input type="text" placeholder="Tìm tên phim..." className="search-input" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
      </div>
      {filteredMovies.length > 0 ? (
          ...
          {filteredMovies.map(movie => (
            <div key={movie.id} className="movie-card">
              <div className="poster-wrapper">
                <img src={movie.image} alt={movie.title} className="movie-poster" onClick={() => setPlayingTrailer(movie.trailer)} style={{ cursor: 'pointer' }} />
                <div className="overlay">
                  <button className="btn-overlay btn-details" onClick={(e) => { e.stopPropagation(); setViewingDetails(movie); }}>CHI TIẾT</button>
                  <button className="btn-overlay btn-buy" onClick={(e) => { e.stopPropagation(); handleBookingStart(movie); }}>MUA VÉ</button>
                </div>
              </div>
        ...
        <div style={{ textAlign: 'center', color: '#a0aec0', marginTop: '50px' }}><p style={{ fontSize: '4rem', margin: '0' }}>🎬</p><p>Không tìm thấy phim nào.</p></div>
      )}
